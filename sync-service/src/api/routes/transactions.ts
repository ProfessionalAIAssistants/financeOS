...
router.get('/', async (req: Request, res: Response) => {
  try {
    const { page = '1', limit = '50', type, account_id } = req.query as Record<string, string>;
    const txns = await getTransactions(parseInt(page), parseInt(limit), type, account_id);

    // Flatten Firefly's nested structure: each "transaction group" contains a transactions array
...
// PUT /api/transactions/:id — update (description, category, tags, notes, amount, date)
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const result = await updateTransaction(req.params.id, payload);
